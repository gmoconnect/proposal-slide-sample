...
> **注意 — 縦積みレイアウト:** kpi レイアウトの内側コンテナは `flex flex-col` で縦積みになっている。`flex-col` が欠落すると SlideLabel・H2・KPI グリッドが**横一列（flex row）**に並んでしまう。また `justify-center` を追加するとコンテンツ全体が垂直中央配置になり、他ページのタイトルより下に見える場合があるため注意。
...
> **注意 — Callout 内の空行:** Callout タグ内に**空行を入れると Slidev が `<p>` タグとして解釈**し、`<p>` のデフォルトマージン（上下 1em）が付加されて余白が過多になる。`Callout.vue` では `[&>p]:my-0` を適用済みなので現在は抑制されているが、Callout 内では空行を入れずにテキストを詰めて書くことを推奨する。
