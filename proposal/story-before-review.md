...
> **本書の構成**: エグゼクティブサマリー（結論）→ 現状と課題 → ソリューション → 実施上の重要ポイント → コストと効果 → 長期戦略 → 補足資料
...
### スライド2: 6つの評価観点すべてにおいて、現行システムから大幅に改善する
...
| 評価観点 | Before（現行） | After（AWS移行後） |
|---|---|---|
| **性能** | ピーク時にリソース不足リスク。固定スペック。 | Fargate/Aurora自動スケーリングでピーク時も安定稼働。CloudFrontで静的コンテンツを高速配信。 |
| **可用性** | 単一データセンター、手動フェイルオーバー | Multi-AZ自動フェイルオーバー、SLA 99.95%以上 |
| **コスト** | ピーク構成の固定費用（通常時は過剰投資） | 従量課金で通常時コスト大幅削減。S3階層化で保管コスト最大92%削減。 |
| **拡張性** | DB 100GB上限、バイナリもDB格納 | S3は容量無制限、Aurora自動拡張。5年後1TBにも対応。 |
| **セキュリティ** | 手動CVE確認、手動パッチ適用 | マネージドWAF + GuardDuty + Distrolessで自動化。CVE対象大幅削減。 |
| **可視化** | KPIダッシュボードなし | QuickSightでリアルタイムダッシュボード |

#### 図の仕様（デザイナー向け）

- レイアウト: 2×3のKPIカード配置
- 各カード: 評価観点名、Before値（グレー）、After値（プライマリカラー）、改善ポイント（アクセントカラー）
- カードにはDiffArrow（改善方向を示す矢印）を付与

---

## セクション2: 対象ビジョンとスコープ定義
...
現行の課題:
- すべてのコンポーネントが単一データセンター内に配置されており、データセンター障害時にサービス全体が停止するリスクがある
- バイナリデータ（画像・PDF等）もすべてRDBMSに格納しており、データ転送がWeb→AP→DBの全層を経由する
...
### スライド4: 現行システムには5つの構造的課題があり、ハードウェア更新だけでは解決できない

#### キーメッセージ

現行システムの課題はハードウェアの老朽化だけではない。アーキテクチャそのものに起因する5つの構造的課題があり、ハードウェアを新調しても同じ問題が再発する。クラウドネイティブアーキテクチャへの刷新が必要である。

#### 本文

**課題1: ハードウェア故障による可用性低下**
- 単一データセンター構成のため、ハードウェア障害がサービス停止に直結する
- 大規模災害時のBCP対応（遠隔地バックアップ）も必要

**課題2: 月末アクセス集中**
- 月間100万アクセスのうち、90%が月末2営業日の営業時間（8時間）に集中
- ピーク時: 約31 req/sec、通常時: 約0.3 req/sec → **約100倍の差**
- ピーク性能に合わせたハードウェア構成のため、通常時は過剰投資となりコストがかさむ
...
### スライド5: AWSサーバーレス・マネージドサービスを中心としたクラウドネイティブアーキテクチャで全課題を解決する

#### キーメッセージ

全体アーキテクチャの概要を示す。サーバーレスとマネージドサービスを最大限活用し、ハードウェアの管理・運用から完全に解放されるNoOpsアーキテクチャを構築する。
...
- **ダッシュボード**: S3ログ → Athena（サーバーレスクエリ） → QuickSight（BIダッシュボード）
- **CI/CD**: CodeCommit → CodeBuild → ECR → CodePipeline（手動承認） → CodeDeploy ECS Blue/Green（TestTraffic付き）
- **セキュリティ監視**: GuardDuty（脅威検出）、CloudTrail（監査ログ）

**設計原則:**
- すべてのコンポーネントはMulti-AZ配置で高可用性を確保する
- サーバーの管理はAWSに委ねる（OS、パッチ適用、スケーリングの自動化）
- バイナリデータはS3 Pre-signed URLでクライアントから直接読み書きし、アプリケーション層の負荷をオフロードする

#### 図の仕様（デザイナー向け）

- レイアウト: フルシステムアーキテクチャ図
- 左（ユーザー）→ 右（データ）の主要フロー:
...
### スライド6: CloudFront + Fargate自動スケーリングにより、月末ピーク時でもレスポンス劣化なく対応する
...
### スライド7: Aurora Serverless v2により、データベースの容量・性能がワークロードに応じて自動調整される
...
### スライド8: S3 Pre-signed URLによりバイナリデータをオフロードし、アプリケーション負荷とDBボトルネックを同時に解消する
...
### スライド9: S3ライフサイクルポリシーにより、5年間のデータ保管コストを最大92%削減する
...
### スライド10: Go言語 + Distrolessイメージにより、セキュリティリスクと運用負荷を最小化する

#### キーメッセージ

コンテナイメージにGo言語の単一バイナリとDistrolessベースイメージを採用する。OS層やミドルウェアがほぼ存在しないため、CVE（脆弱性）の検知対象が大幅に減少する。加えて、rootファイルシステムをread-onlyに設定することで、実行時の改ざんを防止する。
...
### スライド11: WAF・GuardDuty・CloudTrailの多層防御により、セキュリティ監視を自動化する

#### キーメッセージ

AWS WAF、GuardDuty、CloudTrailを組み合わせた多層防御を構築する。すべてマネージドサービスのため、セキュリティ監視の自動化が実現する。現行の手動によるセキュリティ監視・CVE確認・パッチ適用から解放される。
...
**WAF運用オプション:**
- 標準: AWS Managed Rulesで主要な攻撃パターンに対応
- 拡張: WAFのSOC（Security Operation Center）が必要な場合、WafCharmなどのSOCサービスの導入を検討
...
- 下部: Before/After運用チェックリスト（Afterは自動化済みの項目に✓マーク）
...
### スライド12: CI/CDパイプラインとBlue/Greenデプロイにより、ゼロダウンタイムで安全なリリースを実現する
...
### スライド13: Multi-AZ構成と自動フェイルオーバーにより、99.95%以上の可用性を確保する
...
**Before/After:**
- Before: 単一DC構成、手動フェイルオーバー、遠隔地バックアップなし
- After: Multi-AZ自動フェイルオーバー、遠隔地バックアップ完備、SLA 99.95%以上
...
### スライド14: Cognito認証とALBのJWT検証により、セキュアかつスケーラブルな認証基盤を実現する

#### キーメッセージ

Amazon Cognitoでマネージド認証基盤を構築する。ALBレベルでJWT（JSON Web Token）を検証し、認証されていないリクエストをアプリケーション到達前に遮断する。認証基盤自体がスケーラブルなため、月末ピーク時にも認証がボトルネックにならない。
...
**効果:**
- 認証インフラの構築・運用がゼロ（Cognitoがフルマネージド）
- 不正アクセスをALBで遮断 → アプリケーション層の負荷を軽減
- Cognitoはスケーラブル → 月末ピーク時の同時認証にも対応
- MFA対応でセキュリティ強化が容易
...
### スライド15: QuickSightダッシュボードにより、システムKPIをリアルタイムに可視化する
...
**Before/After:**
- Before: KPIダッシュボードなし。問題発生後に手動でログを調査
- After: リアルタイムダッシュボードで常時監視。異常の早期検出と傾向分析が可能
...
## セクション5: AWSランニングコストと効果

---

### スライド16: 従量課金モデルにより、月額ランニングコストを最適化する
...
| サービス | 通常時（月額概算） | ピーク時（月末2日間） | 備考 |
|---|---|---|---|
| ECS on Fargate | 最小タスク構成の課金 | タスク自動増加分のみ追加課金 | vCPU/メモリ × 稼働秒数 |
| Aurora Serverless v2 | 最小ACUでの課金 | ACU自動増加分のみ追加課金 | ACU × 稼働時間 |
| S3 | 格納量に応じた課金 | - | ライフサイクルで階層化済み |
| CloudFront | 転送量に応じた課金 | キャッシュによりオリジン転送量削減 | エッジキャッシュの効果 |
| ALB | 固定費 + LCU課金 | LCU増加分 | LCU = ロードバランサーキャパシティユニット |
| Cognito | MAU（月間アクティブユーザー）課金 | - | 50,000 MAUまで無料枠 |
| WAF | WebACL + ルール数 + リクエスト数 | リクエスト増加分 | Managed Rulesは月額固定 |
| GuardDuty | 分析データ量に応じた課金 | - | 30日無料トライアルあり |
| QuickSight | ユーザー数課金 | - | Author/Reader別の料金体系 |
| CodePipeline/Build/Deploy | パイプライン実行回数課金 | - | 開発・デプロイ時のみ |
...
- レイアウト: コスト内訳の棒グラフまたはウォーターフォールチャート
- 主要サービスごとの月額コスト（通常月 / ピーク月）の比較
- オンプレミスの固定費との比較線を重ねて表示

---

### スライド17: 6つの評価観点における改善効果を定量的に示す

#### キーメッセージ

6つの評価観点すべてにおいて、現行システムから定量的に改善される。各観点でBefore/Afterを明確に示し、提案アーキテクチャの効果を総括する。
...
### スライド18: フロント・バック分離とIaCにより、将来の機能拡張にも柔軟に対応する

#### キーメッセージ

本提案のアーキテクチャはフロントエンドとバックエンドが明確に分離されたAPI構成である。この設計により、将来的なAPI公開やAIエージェントからのMCP（Model Context Protocol）を通じたシステム利活用も視野に入る。Infrastructure as Code（IaC）によりインフラ構成をコード管理し、環境の再現性と変更管理を確保する。
...
- CloudFormation / CDKでインフラ構成をコード管理する
- 環境の再現性: 開発・ステージング・本番を同一コードから構築
- 変更管理: インフラ変更をコードレビュー + CI/CDで統制
- ドリフト検知: 手動変更の検出と是正
...
- 右に拡張矢印:
  - API公開（外部システム連携）
  - AIエージェント連携（MCP）
- 下部: IaCによるインフラ管理フロー（コード → レビュー → デプロイ）
...
### スライド19: 評価項目No.5に対する回答の総括 — AWSサーバーレス構成で6つの評価観点すべてを満たす
...
| 評価観点 | 本提案での回答 | 該当スライド |
|---|---|---|
| **性能** | CloudFront CDN + Fargate/Aurora自動スケーリングにより、月末ピーク（約31 req/sec、通常時の約100倍）でもレスポンス劣化なく対応。S3 Pre-signed URLでバイナリ転送の90%をオフロード。 | 6, 7, 8 |
| **可用性** | 全層Multi-AZ配置 + 自動フェイルオーバーでSLA 99.95%以上。遠隔地バックアップはAurora→AWS Backup、S3→クロスリージョンレプリケーション。Blue/Greenデプロイでゼロダウンタイムリリース。 | 12, 13 |
| **コスト** | 従量課金モデルで通常時コストを大幅削減。S3ライフサイクルで保管コスト最大92%削減。マネージドサービスによる運用人件費の削減。 | 6, 9, 16 |
| **拡張性** | S3は容量無制限、Aurora Serverless v2はACU/ストレージ自動拡張。バイナリをS3にオフロードしDBサイズを1/10に抑制。5年後の1TBデータ増にも対応。 | 7, 8, 9 |
| **セキュリティ** | WAF（Managed Rules）+ GuardDuty + CloudTrailの多層防御。Go Distroless + read-only FSでCVE大幅削減。Cognito + ALB JWT検証で認証を自動化。 | 10, 11, 14 |
| **可視化** | S3ログ → Athena → QuickSightでリアルタイムBIダッシュボードを構築。アクセス数・レスポンスタイム・エラー率・リソース使用率を常時可視化。 | 15 |
...
| その他マネージド | WAF, GuardDuty, QuickSight等 | [要見積] |
